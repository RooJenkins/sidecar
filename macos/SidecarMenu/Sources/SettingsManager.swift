import AppKit
import Foundation
import Observation
import Carbon.HIToolbox
import ServiceManagement

enum HotkeyType: String, CaseIterable, Identifiable {
    case doubleTapOption = "doubleTapOption"
    case keyCombo = "keyCombo"

    var id: String { rawValue }

    var label: String {
        switch self {
        case .doubleTapOption: return "Double-tap Option"
        case .keyCombo: return "Key combination"
        }
    }
}

@Observable
final class SettingsManager {
    static let shared = SettingsManager()

    var enabled: Bool {
        didSet { UserDefaults.standard.set(enabled, forKey: "enabled") }
    }

    var cliPath: String {
        didSet { UserDefaults.standard.set(cliPath, forKey: "cliPath") }
    }

    var maxResults: Int {
        didSet { UserDefaults.standard.set(maxResults, forKey: "maxResults") }
    }

    var indexedFolders: [String] {
        didSet { UserDefaults.standard.set(indexedFolders, forKey: "indexedFolders") }
    }

    var hotkeyType: HotkeyType {
        didSet { UserDefaults.standard.set(hotkeyType.rawValue, forKey: "hotkeyType") }
    }

    /// The virtual key code for the key combo (e.g. kVK_Space = 0x31)
    var hotkeyKeyCode: UInt16 {
        didSet { UserDefaults.standard.set(Int(hotkeyKeyCode), forKey: "hotkeyKeyCode") }
    }

    /// Modifier flags stored as raw UInt (NSEvent.ModifierFlags)
    var hotkeyModifiers: UInt {
        didSet { UserDefaults.standard.set(hotkeyModifiers, forKey: "hotkeyModifiers") }
    }

    var launchAtLogin: Bool {
        didSet {
            do {
                if launchAtLogin {
                    try SMAppService.mainApp.register()
                } else {
                    try SMAppService.mainApp.unregister()
                }
            } catch {
                Logger.log("Launch at login error: \(error)", source: "Settings")
            }
        }
    }

    var autoWatch: Bool {
        didSet { UserDefaults.standard.set(autoWatch, forKey: "autoWatch") }
    }

    var modifierFlags: NSEvent.ModifierFlags {
        NSEvent.ModifierFlags(rawValue: hotkeyModifiers)
    }

    private init() {
        let defaults = UserDefaults.standard

        defaults.register(defaults: [
            "enabled": true,
            "cliPath": "",  // Empty = auto-detect via resolveCLIPath
            "maxResults": 5,
            "indexedFolders": [String](),
            "autoWatch": true,
            "hotkeyType": HotkeyType.keyCombo.rawValue,
            "hotkeyKeyCode": Int(kVK_ANSI_J),  // Default: Cmd+J
            "hotkeyModifiers": Int(NSEvent.ModifierFlags.command.rawValue),
        ])

        self.enabled = defaults.bool(forKey: "enabled")
        self.cliPath = defaults.string(forKey: "cliPath") ?? "/usr/local/bin/sidecar"
        self.maxResults = defaults.integer(forKey: "maxResults")
        self.indexedFolders = defaults.stringArray(forKey: "indexedFolders") ?? []
        self.hotkeyType = HotkeyType(rawValue: defaults.string(forKey: "hotkeyType") ?? "") ?? .keyCombo
        self.hotkeyKeyCode = UInt16(defaults.integer(forKey: "hotkeyKeyCode"))
        self.hotkeyModifiers = UInt(defaults.integer(forKey: "hotkeyModifiers"))
        self.autoWatch = defaults.bool(forKey: "autoWatch")
        // Sync launch-at-login state with actual system state
        self.launchAtLogin = SMAppService.mainApp.status == .enabled
    }

    /// Human-readable description of the current hotkey
    var hotkeyDescription: String {
        if hotkeyType == .doubleTapOption {
            return "Double-tap ⌥"
        }
        var parts: [String] = []
        let flags = modifierFlags
        if flags.contains(.control) { parts.append("⌃") }
        if flags.contains(.option) { parts.append("⌥") }
        if flags.contains(.shift) { parts.append("⇧") }
        if flags.contains(.command) { parts.append("⌘") }
        parts.append(keyCodeToString(hotkeyKeyCode))
        return parts.joined()
    }
}

func keyCodeToString(_ keyCode: UInt16) -> String {
    let map: [UInt16: String] = [
        0x00: "A", 0x01: "S", 0x02: "D", 0x03: "F", 0x04: "H",
        0x05: "G", 0x06: "Z", 0x07: "X", 0x08: "C", 0x09: "V",
        0x0B: "B", 0x0C: "Q", 0x0D: "W", 0x0E: "E", 0x0F: "R",
        0x10: "Y", 0x11: "T", 0x12: "1", 0x13: "2", 0x14: "3",
        0x15: "4", 0x16: "6", 0x17: "5", 0x18: "=", 0x19: "9",
        0x1A: "7", 0x1B: "-", 0x1C: "8", 0x1D: "0", 0x1E: "]",
        0x1F: "O", 0x20: "U", 0x21: "[", 0x22: "I", 0x23: "P",
        0x25: "L", 0x26: "J", 0x27: "'", 0x28: "K", 0x29: ";",
        0x2A: "\\", 0x2B: ",", 0x2C: "/", 0x2D: "N", 0x2E: "M",
        0x2F: ".", 0x30: "Tab", 0x31: "Space", 0x32: "`",
        0x33: "Delete", 0x24: "Return", 0x35: "Esc",
        0x7A: "F1", 0x78: "F2", 0x63: "F3", 0x76: "F4",
        0x60: "F5", 0x61: "F6", 0x62: "F7", 0x64: "F8",
        0x65: "F9", 0x6D: "F10", 0x67: "F11", 0x6F: "F12",
    ]
    return map[keyCode] ?? "Key\(keyCode)"
}
